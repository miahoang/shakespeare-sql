...
import numpy as np
import pandas as pd
from src.dataset import add_nl_interpretations
from src.dataset import merge_all_insert_statements
from src.dataset.config import DataConfig
...
df['db_file_full'] = df['db_file'].apply(
        lambda x: os.path.join(DataConfig.data_dir, 'ambrosia', x) if isinstance(x, str) and not os.path.isabs(x) else x
    )
df['db_dump_processed'] = df.apply(lambda row: merge_all_insert_statements(row['db_file_full'], row['db_dump']), axis=1)
