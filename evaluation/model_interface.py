...
class VLLMModelWrapper(ModelInterface):
    def __init__(self, client, model_name=None):
        self.client = client
        self.model_name = model_name
        self._device = torch.device("cpu")  # vLLM runs on server side

        logging.getLogger("openai").setLevel(logging.ERROR)
        logging.getLogger("httpx").setLevel(logging.ERROR)

    def generate(self, inputs, **kwargs) -> str:
        # Map parameters to vLLM OpenAI-compatible API
        vllm_params = {}
        param_mapping = {
            'temperature': 'temperature',
            'top_p': 'top_p',
        ...
        for src_param, vllm_param in param_mapping.items():
            if src_param in kwargs:
                vllm_params[vllm_param] = kwargs[src_param]
